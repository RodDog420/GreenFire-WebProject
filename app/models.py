...
class Product(db.Model):
    ...
    # Acquisition
    # 'purchased'   = outright purchase, artist no longer involved
    # 'consignment' = artist retains ownership until sold
    acquisition_type = db.Column(db.String(16), nullable=False, default='purchased')
    consignment_rate = db.Column(db.Numeric(5, 2), nullable=True)
    consignment_paid = db.Column(db.Boolean, default=False, nullable=True)
    ...
    @property
    def original_price_dollars(self):
        if self.original_price_cents:
            return self.original_price_cents / 100
        return None
...
class ProductImage(db.Model):
    ...
    alt_text    = db.Column(db.String(256), nullable=False)
...
class Order(db.Model):
    ...
    stripe_payment_intent_id = db.Column(db.String(256), nullable=True, unique=True)
